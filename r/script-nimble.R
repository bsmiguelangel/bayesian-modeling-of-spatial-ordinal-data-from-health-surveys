...
modelCode <- nimbleCode(
  {
    ...
    for (SexGroup in 1:NSex) {
      for (AgeGroup in 1:NAges) {
        ...
        # delta[1:NSex, 1:NAges, 1:NCats] Stick-breaking prior
        delta[SexGroup, AgeGroup, 1:NCats] <- 
          stick_breaking(omega[SexGroup, AgeGroup, 1:(NCats-1)])
      }
    }
...
run_MCMC_allcode <- function(X, code, constants, data, monitors) {
  ...
  pacman::p_load(nimble, install = FALSE)
  ...
  model <- nimbleModel(code = code, 
                       constants = constants, 
                       data = data, 
                       inits = list(omega = array(c(rbeta(NSex * NAges, 1, NCats - 1), 
  ...
  modelMCMCconfiguration <- configureMCMC(model, useConjugacy = FALSE)
  # Remove desire samplers
  modelMCMCconfiguration$removeSamplers(c("omega", 
                                          "alpha", "sd.theta", "theta", "rho"))
  
  # Add slice omega[1:NSex, 1:NAges, 1:(NCats-1)] samplers
  omegas <- array(dim = c(NSex, NAges, NCats - 1))
  for (SexGroup in 1:NSex) {
    for (AgeGroup in 1:NAges) {
      for (Cat in 1:(NCats - 1)) {
        omegas[SexGroup, AgeGroup, Cat] <- paste("omega[",SexGroup,",",AgeGroup,",",Cat,"]")
      }
    }
  }
  omegas <- gsub(" ", "", omegas)
  ...
  for (SexGroup in 1:NSex) {
    for (AgeGroup in 1:NAges) {
      for (Cat in 1:(NCats - 1)) {
        modelMCMCconfiguration$addSampler(target = omegas[SexGroup, AgeGroup, Cat], 
                                          type = "slice")
      }
    }
  }
...
# c("kappa", "alpha", "theta", "sd.theta", "rho")
...
mean((salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "kappa"), 6] > 1.02) | (salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "kappa"), 7] < 400))
mean((salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "alpha"), 6] > 1.02) | (salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "alpha"), 7] < 400))
mean((salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "sd.theta"), 6] > 1.02) | (salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "sd.theta"), 7] < 400))
mean((salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "rho"), 6] > 1.02) | (salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "rho"), 7] < 400))
mean((salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "theta"), 6] > 1.02) | (salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "theta"), 7] < 400))
...
breaks
# breaks <- c(-0.60, round(breaks[2:9], 2), 0.47)
...
carto_muni@data$thetamean <- factor(carto_muni@data$thetamean, levels = 
                                      rev(levels(carto_muni@data$thetamean)))
# levels(carto_muni@data$thetamean) <- c("(0.18,0.47)", levels(carto_muni@data$thetamean)[2:9])
