...
run_MCMC_allcode <- function(X, code, constants, data, monitors) {
  ...
  # Function that returns the value of the joint density function of the CAR Leroux
  dcar_leroux <- nimbleFunction(
    name = 'dcar_leroux',
    run = function(x = double(1),        # Spatial random effect (vector)
                   rho = double(0),      # Amount of spatial dependence (scalar)
  ...
  model <- nimbleModel(code = code, 
                       constants = constants, 
                       data = data, 
                       inits = list(delta = array(rdirichlet(constants$NSex * constants$NAges, 
                                                             constants$ones), 
                                                  dim = c(constants$NSex, 
                                                          constants$NAges, 
                                                          constants$NCats)), 
                                    alpha = c(NA, runif(constants$NDwells - 1)),
                                    rho = runif(1), sd.theta = runif(1),
                                    theta = rnorm(constants$NMuni, sd = 0.01))
                       , calculate = FALSE)
  ...
  # The conjugacy checking is a slow part of MCMC configuration, 
  # skipping it (useConjugacy = FALSE) helps speed
  modelMCMCconfiguration <- configureMCMC(model, useConjugacy = FALSE)
  # Remove desire samplers
  modelMCMCconfiguration$removeSamplers(c("sd.theta", "theta", "rho"))
  
  # Add new sd.theta sampler
  modelMCMCconfiguration$addSampler(target = "sd.theta", type = "slice")
  
  # Add RW-MH theta[1:NMuni] samplers
  ...
  CmodelMCMC <- compileNimble(modelMCMC, project = model,
                              resetFunctions = TRUE)
  # Results
  results <- runMCMC(CmodelMCMC, niter = 8000, nburnin = 500, thin = 15,
                     setSeed = X)
...
# 9.35 min on a server with: niter = 8000, nburnin = 500, thin = 15
# saveRDS(salnimble, file = file.path("results", "a-case-study-nimble.rds"))

#### Model results ####

salnimble <- readRDS(file.path("results", "a-case-study-nimble.rds"))
...
summary(c(salnimble[[1]][, "rho"], 
          salnimble[[2]][, "rho"], 
          salnimble[[3]][, "rho"], 
          salnimble[[4]][, "rho"], 
          salnimble[[5]][, "rho"]))

mean((MCMCsummary(object = salnimble, params = "kappa", round = 4)[, 6]) > 1.10 | (MCMCsummary(object = salnimble, params = "kappa", round = 4)[, 7] < 100))
mean((MCMCsummary(object = salnimble, params = "alpha", round = 4)[, 6]) > 1.10 | (MCMCsummary(object = salnimble, params = "alpha", round = 4)[, 7] < 100))
mean((MCMCsummary(object = salnimble, params = "sd.theta", round = 4)[, 6]) > 1.10 | (MCMCsummary(object = salnimble, params = "sd.theta", round = 4)[, 7] < 100))
mean((MCMCsummary(object = salnimble, params = "rho", round = 4)[, 6]) > 1.10 | (MCMCsummary(object = salnimble, params = "rho", round = 4)[, 7] < 100))
mean((MCMCsummary(object = salnimble, params = "theta", round = 4)[, 6]) > 1.10 | (MCMCsummary(object = salnimble, params = "theta", round = 4)[, 7] < 100))

#### Function: salnimble to salwinbugs ####

# NimToWin: 
# - (1) transforms NIMBLE output to WinBUGS sims.list output format
...
salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "theta"), ]

mean((salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "kappa"), 6] > 1.10) | (salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "kappa"), 7] < 100))
mean((salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "alpha"), 6] > 1.10) | (salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "alpha"), 7] < 100))
mean((salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "sd.theta"), 6] > 1.10) | (salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "sd.theta"), 7] < 100))
mean((salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "rho"), 6] > 1.10) | (salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "rho"), 7] < 100))
mean((salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "theta"), 6] > 1.10) | (salwinbugs$summary[startsWith(labels(salwinbugs$summary)[[1]], "theta"), 7] < 100))
...
breaks
breaks <- c(-0.58, round(breaks[2:9], 2), 0.47)
...
carto_muni@data$thetamean <- factor(carto_muni@data$thetamean, levels = 
                                      rev(levels(carto_muni@data$thetamean)))
levels(carto_muni@data$thetamean) <- c("(0.17,0.47)", levels(carto_muni@data$thetamean)[2:9])
...
breaks
breaks <- c(14.69, round(breaks[2:9], 2), 31.34)
carto_muni@data$category1 <- cut(category, breaks = breaks, include.lowest = FALSE, right = TRUE, dig.lab = 4)
levels(carto_muni@data$category1) <- c("(14.69,17.63]", "(17.63,18.71]", "(18.71,19.44]",
                                       "(19.44,20.18]", "(20.18,21.13]", "(21.13,22.02]",
                                       "(22.02,23.16]", "(23.16,25.02]", "(25.02,31.34)")
category <- percentagesmean[, 2]
breaks <- c(min(category) - 0.001, quantile(category, probs = seq(1/9, 8/9, length.out = 8)), max(category))
breaks
breaks <- c(43.03, round(breaks[2:9], 2), 52.06)
carto_muni@data$category2 <- cut(category, breaks = breaks, include.lowest = FALSE, right = TRUE, dig.lab = 4)
levels(carto_muni@data$category2) <- c("(43.03,47.07]", "(47.07,48.03]", "(48.03,48.51]",
                                       "(48.51,49.05]", "(49.05,49.41]", "(49.41,49.75]",
                                       "(49.75,50.11]", "(50.11,50.50]", "(50.50,52.06)")
category <- percentagesmean[, 3]
breaks <- c(min(category) - 0.001, quantile(category, probs = seq(1/9, 8/9, length.out = 8)), max(category))
breaks
breaks <- c(14.48, round(breaks[2:9], 2), 28.97)
carto_muni@data$category3 <- cut(category, breaks = breaks, include.lowest = FALSE, right = TRUE, dig.lab = 4)
levels(carto_muni@data$category3) <- c("(14.48,18.32]", "(18.32,19.58]", "(19.58,20.37]",
                                       "(20.37,21.19]", "(21.19,22.07]", "(22.07,22.72]",
                                       "(22.72,23.46]", "(23.46,24.58]", "(24.58,28.97)")
category <- percentagesmean[, 4]
breaks <- c(min(category) - 0.001, quantile(category, probs = seq(1/9, 8/9, length.out = 8)), max(category))
breaks
breaks <- c(3.29, round(breaks[2:9], 2), 9.75)
carto_muni@data$category4 <- cut(category, breaks = breaks, include.lowest = FALSE, right = TRUE, dig.lab = 4)
levels(carto_muni@data$category4) <- c("(3.29,4.68]", "(4.68,5.19]", "(5.19,5.53]",
                                       "(5.53,5.91]", "(5.91,6.27]", "(6.27,6.58]",
                                       "(6.58,6.85]", "(6.85,7.53]", "(7.53,9.75)")
...
levels(carto_muni@data$category5) <- c("(1.19,1.78]", "(1.78,2.01]", "(2.01,2.15]",
                                       "(2.15,2.32]", "(2.32,2.46]", "(2.46,2.60]",
                                       "(2.60,2.74]", "(2.74,2.98]", "(2.98,3.78)")
...
# Saving the table
saveRDS(result2, file = file.path("results", "model-assessment-nimble.rds"))
