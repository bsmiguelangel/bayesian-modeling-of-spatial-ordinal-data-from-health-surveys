...
# index.groupD contains the number of respondents sorted by dwelling stratum
index.groupD <- as.numeric(table(dwell))
index.groupD <- as.vector(c(0, cumsum(index.groupD)))

# groupD contains the respondents sorted by dwelling stratum
groupD <- c()
...
model <- function() {
  ...
  # Constraint for theta - dwelling stratum covariate
  for (Dwell in 1:NDwells) {
...
#### Supplementary information: Dwelling stratum effect is not relevant ####
...
#### Figure 3.1: Map of the RV - theta[1:NMuni] ####
...
#### Figure 3.2: Map of the RV - P(theta < 0 | y) ####
...
#### Figure 4: Maps of the RV - real percentages with post-stratification ####
