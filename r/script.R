...
pacman::p_load(foreign, readxl, faraway, R2WinBUGS, pbugs, spdep, maptools, 
               ggplot2, RColorBrewer, graphics, ggpubr, leaflet, install = FALSE)
...
# Covariate age group: 1 = [15,45); 2 = [45,65); 3 = [65,75); 4 = [75,85);
# 5 = [85,...)
...
# Loading the population of each municipality by sex and age group:
...
### Preparation of maps: Option 2 ###

# Cartography of the Region of Valencia
load(file.path("data", "CartoCV.Rdata"))
# Cartography sorted by municipality code, vector of 0s
...
map <- unlist(cv.nb)
# Sum of all the neighbor numbers of all the municipalities
nadj.tot <- length(map)
...
model <- function() {
  ...
  # Definition of delta in logarithmic scale
  for (SexGroup in 1:NSex) {
    for (AgeGroup in 1:NAges) {
  ...
  for (Muni in 1:NMuni) {
    ...
    prec.theta[Muni] <- (1 - lambda + lambda * nadj[Muni])
  ...
  # Constraint for theta - (sex, age group) covariates
  ...
  # Constraint for theta - dwelling covariate
  for (Dwell in 1:NDwells) {
...
data <- list(y = y, NResp = NResp, NCats = NCats, sex = sex, age = age, dwell = dwell, 
             NSex = NSex, NAges = NAges, NDwells = NDwells, NGroupSA = NGroupSA, 
             groupSA = groupSA, groupD = groupD, index.groupSA = index.groupSA, 
             index.groupD = index.groupD, muni = muni, NMuni = NMuni, nadj = nadj, 
...
salwinbugs <- pbugs(model = model, data = data, inits = inits, parameters.to.save = parameters, 
                    n.chains = 5, n.iter = 6000, n.burnin = 1000, n.thin = 25,
                    #working.directory = "~/.wine/drive_c/temp/Rtmp",
                    working.directory = "/home/beltran_mig/.wine/drive_c/users/beltran_mig/Temp/Area1/",
                    cluster = 5, bugs.seed = 3687241
                    #, debug = TRUE
                    , clearWD = TRUE
                    , DIC = FALSE
)

# saveRDS(salwinbugs, file = file.path("results", "a-case-study.rds"))

#### Model results ####

salwinbugs <- readRDS(file.path("results", "a-case-study.rds"))
...
#### Supplementary information: Dwelling effect is not relevant ####
...
suppl <- ggarrange(p[[1]], p[[2]], p[[3]], p[[4]], nrow = 2, ncol = 2)
suppl

# Saving the plot
ggsave("suppl.eps", device = cairo_ps, fallback_resolution = 600,
...
#### Figure 3.1: Map of the VC - theta[1:NMuni] ####
...
#### Figure 3.2: Map of the VC - P(theta < 0 | y) ####
...
#### Figure 4: Maps of the VC - real percentages with post-stratification ####

# function1: 
# - (1) computes the 1000 simulated probabilities for each sex, age group and municipality
# - (2) post-stratifies these probabilities for each municipality and category

function1 <- function(salwinbugs) {
  
  paramsim <- salwinbugs$sims.list
  ...
  for (sim in 1:n.sims) {
        ...
        for (Muni in 1:NMuni) {
          for (Cat in 1:(NCats - 1)) {
            p.gamma[sim, Muni, SexGroup, AgeGroup, Cat] <- 
              ilogit(paramsim$kappa[sim, SexGroup, AgeGroup, Cat] + 
                       paramsim$sd.theta[sim] * paramsim$theta[sim, Muni])
          }
    ...
    if (sim %in% c(1, seq(200, 1000, 200))) {
      cat(sim, "of", 1000, "simulations", "\n")
    } else {}
  }
...
### Posterior mean of the percentages for each municipality and category ###
...
# from function1; that is, results1$prlevels.
...
function2 <- function(prlevels, Muni) {
  ...
  CInterval0.025 <- round(apply(predictive.muni, 2, quantile, prob = 0.025), 2)
  CInterval0.975 <- round(apply(predictive.muni, 2, quantile, prob = 0.975), 2)
  ...
  return(list("mean" = posteriormean,
              "CI" = list("lower" = CInterval0.025, "upper" = CInterval0.975),
              "real" = realvalue))
}
...
# Municipalities with the largest sample sizes
Munis <- order(apply(sample, 1, sum), decreasing = TRUE)[1:4]
...
# Saving the table
saveRDS(result2, file = file.path("results", "model-assessment.rds"))
